    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuInfo.skuImageList?skuInfo.skuImageList:[{}]" />
          <!-- 小图列表 -->
          <ImageList :skuImageList="skuInfo.skuImageList?skuInfo.skuImageList:[{}]" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active:spuSaleAttrValue.isChecked==='1'}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"
                  @click="changActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="plus" @click="increaseSkuNum">+</a>
                <a href="javascript:" class="mins" @click="decreaseSkuNum">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...
    // 商品数量过滤函数
    changeSkuNum() {
      this.skuNum < 1 ? this.skuNum = 1 : this.skuNum = parseInt(this.skuNum)
    },
    // 增加商品
    increaseSkuNum() {
      this.skuNum++
    ...
    async addShopCar() {
      // 派发添加购物车
      try {
        await this.$store.dispatch('detail/addOrUpdataShopCar', { skuId: this.$route.params.goodId, skuNum: this.skuNum })
        this.$router.push({
          name: 'addcartsuccess',
          query: {
            skuNum: this.skuNum
